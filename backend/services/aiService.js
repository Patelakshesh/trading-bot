...
const getGlobalTop5TradingTips = async (news, movers, budget = null, priceRange = null, niftyChange = null) => {
    try {
        ...
        let rangePrompt = "";
        if (priceRange) {
            rangePrompt = `CRITICAL: The user requested stocks priced strictly between ₹${priceRange.min} and ₹${priceRange.max}. You MUST ONLY recommend stocks within this exact price range. If the provided movers do not fit this range, you must generate 5 other HIGH-BETA, HIGHLY VOLATILE momentum stocks (Strategy B) that fit this price bracket. NEVER recommend slow blue-chips just to fill the list.`;
        }
...
${movers.gainers.slice(0, 15).map(g => `- ${g.symbol} (${g.name}): ₹${g.price} (+${g.changePercent}%) [ADX: ${g.adx || 'N/A'}] [Trend: ${g.trend || 'N/A'}]`).join('\n')}

TOP NSE LOSERS (Bounce-back candidates):
${movers.losers.slice(0, 15).map(l => `- ${l.symbol} (${l.name}): ₹${l.price} (${l.changePercent}%) [ADX: ${l.adx || 'N/A'}] [Trend: ${l.trend || 'N/A'}]`).join('\n')}
